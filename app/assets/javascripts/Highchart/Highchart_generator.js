...
//var Highchart_generator=function(){
    // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> daily_dpv
...
var Highchart_dpv=function(option){
    this.basic=deepCopy(Highchart_generator.basic,{});
    this.init=function(){
        var $target=option.target.indexOf("#")===-1?$("#"+option.target):$(option.target);
        this.basic.title.text=option.title?option.title:null;
        this.basic.subtitle.text=option.subtitle?option.subtitle:null;
        ...
        if(option.color){
            this.basic.colors[0]=Highchart_generator.color_system[option.color]?Highchart_generator.color_system[option.color]:Highchart_generator.color_system["blue"];
        }
        $target.highcharts(this.basic);
    };
    this.reload_daily_dpv=function(another_option){
        var $target=option.target.indexOf("#")===-1?$("#"+option.target):$(option.target);
        $target.highcharts().destroy();
        if(another_option.xArray){
            this.basic.xAxis.categories=another_option.xArray;
        }
        this.basic.series=another_option.data;
        $target.highcharts(this.basic);
    };
