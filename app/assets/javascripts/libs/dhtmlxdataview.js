...
dhtmlx.PowerArray = {removeAt: function (b, a) {
    if (b >= 0) {
        this.splice(b, (a || 1))
    }
}, remove: function (a) {
...
dhtmlx.html = {create: function (b, a, c) {
    a = a || {};
    ...
    if (d.getBoundingClientRect) {
        ...
        var k = g.top + a - f;
        var c = g.left + e - j;
        return{y: Math.round(k), x: Math.round(c)}
    } else {
        var k = 0, c = 0;
        while (d) {
...
dhtmlx.DataDriver = {};
dhtmlx.DataDriver.json = {toObject: function (data) {
    if (!data) {
        data = "[]"
    }
...
dhtmlx.DataDriver.json_ext = {toObject: function (data) {
    if (!data) {
        data = "[]"
    }
    if (typeof data == "string") {
...
dhtmlx.DataDriver.html = {toObject: function (b) {
    if (typeof b == "string") {
        var a = null;
        if (b.indexOf("<") == -1) {
            a = dhtmlx.toNode(b)
...
dhtmlx.DataDriver.jsarray = {toObject: function (data) {
    if (typeof data == "string") {
        eval("dhtmlx.temp=" + data);
        return dhtmlx.temp
    }
...
dhtmlx.DataDriver.xml = {toObject: function (b, a) {
...
dhtmlx.DataStore.prototype = {setDriver: function (a) {
    ...
    if (b) {
        this.mapEvent({onbeforesort: d, onaftersort: d, onbeforeadd: d, onafteradd: d, onbeforedelete: d, onafterdelete: d, onbeforeupdate: d})
    }
        ...
        if (!d && a.getAttribute) {
            ...
            if (h) {
                if (a.getAttribute("userdata")) {
                    this.callEvent("onLocateData", [h, a])
                }
...
dhtmlx.Type = {add: function (c, b) {
    ...
    return"..."
}, width: 150, height: 80, margin: 5, padding: 0}, _template: function (c, a) {
    a = "template" + (a ? ("_" + a) : "");
    var b = c[a];
        ...
        if (b.indexOf("->") != -1) {
            ...
            switch (b[0]) {
                case"html":
                    ...
                    break;
                case"http":
                    b = new dhtmlx.ajax().sync().get(b[1], {uid: (new Date()).valueOf()}).responseText;
                    break;
                default:
                    break
            }
...
dhtmlx.AutoTooltip = {tooltip_setter: function (b) {
...
(function () {
    if (!window.dhtmlxError) {
        ...
        window.dhtmlxError = {catchError: b, throwError: b};
        ...
        var a = {getXMLTopNode: function (c) {
        }, doXPath: function (c) {
            return dhtmlx.DataDriver.xml.xpath(this.xml, c)
        }, xmlDoc: {responseXML: true}};
        dhtmlx.compat.dataProcessor = function (f) {
            var c = "_sendData";
            var d = "_in_progress";
            var g = "_tMode";
            var e = "_waitMode";
            f[c] = function (j, k) {
                ...
                if (k) {
                    this[d][k] = (new Date()).valueOf()
                }
                if (!this.callEvent("onBeforeDataSending", k ? [k, this.getState(k)] : [])) {
                    return false
                }
                var h = this;
                var l = this.serverProcessor;
                if (this[g] != "POST") {
                    dhtmlx.ajax().get(l + ((l.indexOf("?") != -1) ? "&" : "?") + this.serialize(j, k), "", function (o, m, n) {
                        a.xml = dhtmlx.DataDriver.xml.checkResponse(o, m);
                        h.afterUpdate(h, null, null, null, a)
                    })
                } else {
                    dhtmlx.ajax().post(l, this.serialize(j, k), function (o, m, n) {
                        a.xml = dhtmlx.DataDriver.xml.checkResponse(o, m);
                        h.afterUpdate(h, null, null, null, a)
                    })
                }
...
if (window.dhtmlXGridObject && !dhtmlXGridObject.prototype._init_point_connector) {
    ...
    dhtmlXGridObject.prototype._parseHead = function (b, q, m) {
        this._parseHead_connector.apply(this, arguments);
        if (!this._colls_loaded) {
            var n = this.xmlLoader.doXPath("./coll_options", arguments[0]);
            for (var h = 0; h < n.length; h++) {
                var l = n[h].getAttribute("for");
                var o = [];
                var d = null;
                if (this.cellType[l] == "combo") {
                    d = this.getColumnCombo(l)
                } else {
                    if (this.cellType[l].indexOf("co") == 0) {
                        d = this.getCombo(l)
                    }
                }
                var g = this.xmlLoader.doXPath("./item", n[h]);
                var a = [];
                for (var e = 0; e < g.length; e++) {
                    var c = g[e].getAttribute("value");
                    if (d) {
                        var k = g[e].getAttribute("label") || c;
                        if (d.addOption) {
                            a.push([c, k])
                        } else {
                            d.put(c, k)
                ...
                if (a.length) {
                    ...
                    if (o.length && !d) {
                        if (this.registerCList) {
                            this.registerCList(l * 1, o)
                        }
                    }
                }
                if (this._con_f_used[l * 1]) {
                    this._con_f_used[l * 1] = o
                }
            }
...
dhtmlx.ui.pager.prototype = {_id: "dhx_p_id", on_click: {dhx_pager_item: function (a, b) {
...
}, types: {"default": {template: dhtmlx.Template.fromHTML("{common.pages()}"), pages: function (c) {
    var b = "";
    if (c.page == -1) {
        return""
    }
    ...
    for (var a = (c.min || 0); a <= c.max; a++) {
        b += this.button({id: a, index: (a + 1), selected: (a == c.page ? "_selected" : "")})
    }
    return b
}, page: function (a) {
    return a.page + 1
...
dhtmlx.DataProcessor = {_dp_init: function (b) {
    var a = "_methods";
...
dhtmlx.compat.dnd = function () {
    if (window.dhtmlDragAndDropObject) {
        ...
        dhtmlx.DragControl._startDrag = function () {
            ...
            g.dragStartNode = {parentNode: {}, parentObject: {idd: l, id: (k.length == 1 ? k[0] : k), treeNod: {object: m.from}}};
...
dhtmlx.DataMove = {_init: function () {
    ...
    if (c instanceof Array) {
        for (var d = 0; d < c.length; d++) {
            var a = (b || this).indexById(this.move(c[d], g, b, dhtmlx.uid()));
            if (c[d + 1]) {
...
dhtmlx.DragControl = {_drag_masters: dhtmlx.toArray(["dummy"]), addDrop: function (b, c, a) {
    b = dhtmlx.toNode(b);
...
}, top: 5, left: 5, onDragIn: function (c, b, d) {
    ...
    dhtmlx.DragControl._drag_context = {source: b, from: b};
...
dhtmlx.DragItem = {_init: function () {
    ...
    if (g) {
        ...
        var a = dhtmlx.DragControl._drag_context = {source: d, start: g};
        a.from = this;
...
dhtmlx.EditAbility = {_init: function (a) {
    this._edit_id = null;
...
dhtmlx.VirtualRenderStack = {_init: function () {
    ...
    for (var b = (e || 0); b <= d; b++) {
        ...
        if (this.order.length > b) {
            if (!c) {
                ...
                c = {id: this.order[b], $template: "loading"};
    ...
    var a = Math.max(this._dataobj.scrollWidth, this._dataobj.offsetWidth) - b;
    ...
    return{_from: c, _height: j, _top: g, _max: f, _y: d.y, _dx: l}
}};
dhtmlXDataView = function (a) {
    ...
    this._parseSettings(a, {drag: false, edit: false, select: "multiselect", type: "default"});
    ...
    if (this.stopEdit(false, b)) {
        if (this._settings.select) {
            if (this._settings.select == "multiselect") {
                this.select(b, a.ctrlKey, a.shiftKey)
            } else {
                this.select(b)
            }
        }
