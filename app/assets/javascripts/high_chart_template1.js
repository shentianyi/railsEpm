...
function add_series(option) {
    ...
    var color=series_colors[MOD(series_id,series_colors.length)];
...
function proper_type_for_chart(){
    ...
    if(this.type=="pie"){
        ...
        var data=[],dataItem,dataItemValue,chart_name;
        if(this.count==1){
            for(var i=0;i<this.chart.series[0].processedYData.length;i++){
                dataItem=[];
                dataItem.push(this.chart.series[0].data[i].name);
                dataItem.push(this.chart.series[0].processedYData[i]);
                data.push(dataItem);
