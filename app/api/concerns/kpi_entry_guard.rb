require 'rack/oauth2'

module KpiEntryGuard
  PARAM_KEYS=[:email, :kpi_id, :date, :value]
  extend ActiveSupport::Concern

  included do |base|
    helpers HelperMethods
    install_error_responders(base)
  end

  module HelperMethods
    def validate_params_integrated params, keys, raise_error=true
      p=params
      messages=[]
      keys.each do |k|
        if params[k].blank?
          messages<< "#{k} cannot be blank"
        end
        p[k]=params[k]
      end
      raise MultiArgumentError, messages if messages.length>0
      p
    end

    def validate_multi_params_integrated params, keys, raise_error=true
      messages=[]
      p_params=[]
      params.each do |param|
        message={}
        begin
          p_params<< validate_params_integrated(param, keys, raise_error)
        rescue MultiArgumentError => e
          message[param[:data_key]]=e.message
          messages<<message
        end
      end
      raise MultiArgumentError, messages if messages.length>0
      p_params
    end

    # validate single input
    def guard_entry!

      entry_params=validate_params_integrated(params, PARAM_KEYS)

      unless entry_params[:kpi_properties].is_a?(Hashie::Mash)
        entry_params[:kpi_properties] = JSON.parse(entry_params[:kpi_properties])
      end if entry_params[:kpi_properties]

      vc= KpiEntryValidatorCollection.new
      entry_params[:validator_collection]=vc
      validator=KpiEntryValidator.new(entry_params)
      validator.validate #if params[:entry][:validate]==false
      if validator.valid
        yield(vc) if block_given?
      else
        raise ArgumentError, validator.invalid_messages
      end

    end

    # validate batch entries
    def guard_entries!(in_batch=true)

      raise ArgumentError, 'has no data' unless params[:entries].is_a?(Array)
      raise ArgumentError, 'has no data' unless params.has_key?(:entries)

      vc= KpiEntryValidatorCollection.new(params[:has_data_key])

      unless params[:has_data_key]
        indexes=[]
        params[:entries].each_with_index do |entry, i|
          if entry_p=validate_params_integrated(entry, PARAM_KEYS, true)
            entry_p[:validator_collection]=vc
            KpiEntryValidator.new(entry_p)
          else
            indexes<<i+1
          end
        end

        if indexes.size>0
          raise ArgumentError, "data index:#{indexes.join(',')} Argument Error}"
        else
          vc.validators.each do |v|
            v.validate
            vc.valid=false unless v.valid
          end

          if vc.valid
            yield(vc) if block_given?
          else
            unless in_batch
              yield(vc) if block_given?
              raise(ArgumentError, vc.invalid_message)
            else
              raise(ArgumentError, vc.invalid_message)
            end
          end
        end
      else
        entries=validate_multi_params_integrated(params[:entries], PARAM_KEYS, true)

        entries.each{|entry|
          entry[:validator_collection]=vc
          KpiEntryValidator.new(entry)
        }

        vc.validators.each do |v|
          v.validate
          vc.valid=false unless v.valid
        end

        if vc.valid
          yield(vc) if block_given?
        else
           raise MultiArgumentError,vc.invalid_message
        end
      end
    end
  end

  module ClassMethods
    private
    def install_error_responders(base)
      errors=[ArgumentError, MultiArgumentError]
      base.send :rescue_from, *errors, kpi_entry_error_handler
    end

    def kpi_entry_error_handler
      Proc.new { |e|
        response=case e
                   when ArgumentError
                     KpiEntryArgumentError.new(e.message)
                   when MultiArgumentError
                     KpiEntryArgumentError.new(e.message)
                 end
        response.finish
      }
    end
  end

  class KpiEntryError<StandardError
    # description is array
    attr_accessor :status, :error, :description, :uri, :realm

    def initialize(status, error, description = nil, options = {})
      @status = status
      puts "&&&&&&&&&&&&&&&#{error.class}"
      @error = (error.is_a?(Array) || error.is_a?(Hash)) ? error : [error.to_s]
      @description = description
      @uri = options[:uri]
      @realm = options[:realm]
      super [error, description].compact.join(' :: ')
    end

    def protocol_params
      {
          result_code: '0',
          msg: @error
      }
    end

    def finish
      response = Rack::Response.new
      response.status = status
      yield response if block_given?
      unless response.redirect?
        response.header['Content-Type'] = 'application/json'
        response.write MultiJson.dump(compact_hash(protocol_params))
      end
      response.finish
    end

    def compact_hash(hash)
      hash.reject do |key, value|
        value.blank?
      end
    end

  end


  class KpiEntryArgumentError<KpiEntryError
    def initialize(error = :kpi_entry_argument_error, description = nil, options = {})
      super 200, error, description, options
    end
  end

  class MultiArgumentError<StandardError
    def initialize(message=nil)
      @message=message
    end

    def message
      @message
    end
  end
end
