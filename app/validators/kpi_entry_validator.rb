...
class KpiEntryValidator
  attr_accessor :entry_id, :email, :department_name, :kpi_id, :kpi_name, :frequency, :date, :value, :entry_at, :user_kpi_item_id, :value, :item_cache_key, :valid, :content, :entry_type
  attr_accessor :validator_collection, :user_kpi_item, :department, :kpi, :valid_by_cache
  ...
  def initialize args={}
    ...
    self.item_cache_key="kpi_entry_validator:#{self.department_name}:#{self.kpi_id}:#{self.date}:#{self.value}"
...
#    end
    ...
    if kpi=Kpi.find_by_id(self.kpi_id)
      self.kpi=kpi
      if department=kpi.tenant.departments.find_by_name(self.department_name)
        self.department = department
      else
        self.valid=false
        self.content<<I18n.t('vali_msg.invalid_department')
      end
    else
      self.valid=false
      self.content<<I18n.t('vali_msg.invalid_kpi')
...
# end
  ...
  def prepare_params
    ...
    self.entity_id=source.department.entities.first.id
    self.target_max=source.kpi.target_max
    self.target_min=source.kpi.target_min
  ...
  def params_to_hash
    {"base_attrs" => {"entry_id" => self.entry_id,
                      "original_value" => self.value,
                      "kpi_id" => self.kpi_id,
                      "frequency" => self.frequency,
                      # "user_kpi_item_id" => self.user_kpi_item_id,
